...
import { X, RotateCcw, List, Shuffle } from 'lucide-react';
...
export default function QuestionsModal({ onSelectQuestion, onClose }: QuestionsModalProps) {
  const [showAll, setShowAll] = useState(false);
  const [randomQuestion, setRandomQuestion] = useState(QUESTIONS[0]);

  const getRandomQuestion = () => {
    const random = Math.floor(Math.random() * QUESTIONS.length);
    setRandomQuestion(QUESTIONS[random]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full mx-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Вопросы для размышления</h3>
          <button
            onClick={onClose}
